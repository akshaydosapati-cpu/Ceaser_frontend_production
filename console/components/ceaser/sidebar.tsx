...
export function Sidebar() {
  const { currentPage, setCurrentPage, guestDemo } = useApp()
  ...
  return <aside className="ceaser-global-sidebar flex h-full w-[260px] shrink-0 flex-col border-r border-white/[0.08] bg-[#050810] text-white">
    ...
    <div className="px-3 pb-2">
      ...
      <NavButton icon={FileText} label="Library" locked={guestDemo} active={currentPage === "files"} onClick={() => setCurrentPage("files")} />
      <NavButton icon={FolderKanban} label="Projects" locked={guestDemo} active={currentPage === "projects"} onClick={() => setCurrentPage("projects")} />
      <NavButton icon={Puzzle} label="Plugins" locked={guestDemo} active={currentPage === "integrations"} onClick={() => setCurrentPage("integrations")} />
      <NavButton icon={MoreHorizontal} label="More" locked={guestDemo} onClick={() => setShowMore((value) => !value)} />
      {showMore && !guestDemo && <div className="ml-3 border-l border-white/10 pl-2"><NavButton compact icon={Settings} label="Settings" onClick={() => setCurrentPage("settings")} /></div>}
    </div>
